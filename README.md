### Hi, I'm Mubina 👋

I'm an Economics and Data Analytics graduate from the University of Mississippi (B.S., May 2026, Honors College, 3.93 GPA). I work at the intersection of **applied economics, data analysis, and policy research** — combining quantitative tools with real-world context to make sense of complex problems.

---

#### What I'm working on

- 📊 **Building out my data analytics portfolio** — SQL + dashboard work, econometric analysis, and policy briefs (more repos coming soon).

---

#### 🛠️ Tech & tools

**Languages:** Python (pandas, NumPy, statsmodels, matplotlib) · R (tidyverse) · SQL  
**Data & Viz:** Tableau · Power BI · Excel  
**Methods:** Econometrics · Regression analysis · Hypothesis testing · Forecasting · Experimental design · Causal inference  
**Other:** Git/GitHub · Jupyter · VS Code · LaTeX

---

#### 🌐 Languages I speak

English (Fluent) · Uzbek (Native) · Russian (Conversational) · Turkish (Conversational)

---

#### 📫 Reach me

- 📧 mubinamirzayeva31@gmail.com
- 💼 [LinkedIn]((https://www.linkedin.com/in/[your-handle]](https://www.linkedin.com/in/mubina-mirzaeva-2a14aa266/))
- 📍 Open to roles in Chicago, Boston, NYC, SF, DC, Virginia, and remote 
